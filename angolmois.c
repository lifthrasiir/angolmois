...
_o(c_ s){OPENFILENAME o={76,0,0,b,0,0,0,s,512,0,0,0,"Choose a file to play",OFN_HIDEREADONLY,0,0,0,0,0,0};sd("$T)>C+7<PW7@VHY;C/X>X,Z;H$E4LT9<CCE-Q+9>VP&f(H$g(OH%#s5##}]iQ#7'AW8I>G8)<C#|NH$h}H$#6R\\Y<Q`)@H$}NO$hzO$33OT)geV$ffM#fdM###",b);GetOpenFileName(&o);}
#else
#include <dirent.h>
_ __=47;_o(c_ s){}
...
B(_ i,double t,_ m,_ j){b_ x={t,m,j};N[i]++[C[i]=realloc(C[i],sizeof(b_)*(N[i]+3))]=x;}
...
_ Bp() {
	...
	if(f = fopen(bP, "r")) {
		...
		ln = x + 2;
		for(i=0; i<18; i++)
			if(p[i]) B(i, bv[3]==2 && q[i]+1<x ? q[i]+1 : ln-1, 2, 0);
...
S_ gs(_ w,_ h){return SDL_CreateRGBSurface(SDL_SWSURFACE,w,h,32,255<<16,65280,255,0);}
R_ R(_ x,_ y,_ w,_ h){R_ r=gR+gr++%2;r->x=x;r->y=y;r->w=w;r->h=h;return r;}
G(S_ c,R_ r,S_ d,_ x,_ y){d?SDL_BlitSurface(c,r,d,R(x,y,0,0)):c?y?SDL_SetColorKey(c,SDL_SRCCOLORKEY|SDL_RLEACCEL,0):SDL_FillRect(c,r,x):SDL_SetClipRect(s,r);}
_ gi(_ x,_ y){return((x<y?y*y-2*x*x+x*x/y*x:x<y*2?4*y*y-8*x*y+5*x*x-x*x/y*x:0)<<11)/y/y;}
gf(S_ x){if(x)SDL_FreeSurface(x);}
_ gt(_ n){return n?n>1?!bM||SMPEG_status(bM)-SMPEG_PLAYING:SDL_Flip(s):SDL_GetTicks();}
...
_ xx() {
	_ i = 0;
	while(gv()) i |= V==SDL_QUIT || (V==SDL_KEYUP && v==27);
	return i;
}
...
_ mi() {
	...
	if(SDL_Init(0x30)<0 || !(s = SDL_SetVideoMode(800, 600, 32, o[2]<<31)) || Mix_OpenAudio(44100, A.format, 2, 2048)<0) return 1;
	SDL_ShowCursor(0);
...
_ mP() {
	...
	t = gt(0);
	x = po + (t<pp ? 0 : pp ? (pt=t, pp=0) : (t-pt)*bb/ph/24e4);
	z = (_)(x + 1);
	if(x > -1 && ph - T[z]) {
	...
	while(gv()) {
		if(V == SDL_QUIT || (V == SDL_KEYUP && v == 27)) return 2;
		if(V == SDL_KEYDOWN && v/2==142)
			pa=1,pS+=v%2?
				pS<1?.2:pS<5?.5:pS<10?1:pS<95?5:0:
				pS>10?-5:pS>5?-1:pS>1?-.5:pS>.201?-.2:0;
		for(i=0; !*o && i<18; i++)
			if(I[i] >= 0 && v == tk[i]) {
				if(V == SDL_KEYUP && Pu[i]) {
					Pu[i] = 0;
	...
	if(x>ln ? gt(2) && Mix_GroupNewer(1)<0 : x<-1) return 1;

	G(s, R(0,30,J,490), 0, 0x404040, 0);
	G(0, R(0,30,800,490), 0, 0, 0);
	for(i=0; i<18; i++)
		if(I[i] >= 0) {
			G(s, R(I[i],30,tw[i],490), 0, 0, 0);
			if(!*o && SDL_GetKeyState(0)[tk[i]])
				G(S, R(I[i],140,tw[i],380), s, I[i], 140);
			for(j=P[i]; j<Pr[i]; j++) {
				k = jp(x, C[i][j].t) - 5;
				m = C[i][j].m;
				m ^= -(m<0);
				if(m - 1) {
					...
					k = k<30 ? 30 : k;
					if(l>k) G(S, R(800+I[i%9],0,tw[i%9],l-k), s, I[i], k);
				}
			}
			if(P[i]==Pr[i] && Pr[i]<N[i] && C[i][Pr[i]].m==2)
				G(S, R(800+I[i%9],0,tw[i%9],490), s, I[i], 30);
		}
	for(i=z-1; i<y; i++)
		G(s, R(0,jp(x,i),J,1), 0, 0xc0c0c0, 0);
	if(t < ST) {
	...
	G(0, 0, 0, 0, 0);
	if(Pv > 0 || (Pv < 0 && t >= PB)) {
		G(s, R(ta,172,256,256), 0, 0, 0);
		for(i=0; i<4; i++)
			if(i/2 == (t<PB) && Pb[i] >= 0 && bI[Pb[i]])
				G(bI[Pb[i]], R(0,0,256,256), s, ta, 172);
		Pv = -(t < PB);
	}
...
_ main(_ c, char **v) {
	...
	if(c > 1 || *b) {
		...
		if(d = o[3]) {
			...
			for(i=0; i<18; i++)
				m[i] = I[i]>=0 && !(d-3 && d-5 && (i+2)%9>6) ? i : -1, p[i] = i;
			for(i=j=0; i<18; i++)
				...
				for(r=1; r; ) {
					...
					for(i=0; i<x; i++)
						if(z[i] && C[m[i]][q[m[i]]].t < e + 1e-9) {
							...
							n[j]++[l[j] = realloc(l[j], sizeof(b_) * (n[j]+2))] = C[m[i]][q[m[i]]++];
						}
					for(i=0; i<x; i++)
						if(r) u[i] *= u[i] != 2;
		...
		F(s, 320, 284, 2, "loading...", 0x202020, 0x808080);
		gt(1);
		...
		if(o[1]) {
			...
			sprintf(b, "Level %d | BPM %.2f%s | %d note%s [%dKEY%s]", bv[1], bb, "?"+!(xf&8), xn, "s"+(xn==1), (xf&1 ? 7 : 5) * (*bv > 1 ? 2 : 1), xf&2 ? "-LN" : "");
		...
		for(i=0; i<1296; i++) {
			...
			if(bs[i]) {
				...
				j = sl(bs[i]);
				j = j>3 ? *(Uint32*)(bs[i]+j-4) | 0x20202020 : 0;
		...
		for(i=0; i<18; i++)
			if(I[i] >= 0) {
				for(j=140; j<520; j++)
					G(S, R(I[i],j,tw[i],1), 0, gb(tc[i], 0, j-140, 1000), 0);
				for(j=0; i<9 && j*2<tw[i]; j++)
					G(S, R(800+I[i]+j,0,tw[i]-2*j,600), 0, gb(tc[i], 0xffffff, tw[i]-j, tw[i]), 0);
			}
		for(j=-244; j<556; j++) {
			for(i=-10; i<20; i++)
		...
		if(!*o)
			G(S, R(0,584,368,16), 0, 0x404040, 0),
			G(S, R(4,588,360,8), 0, 0, 0);
		G(S, R(10,564,J,1), 0, 0x404040, 0);
		G(s, 0, 0, 0, 0);
